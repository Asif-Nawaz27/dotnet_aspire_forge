...
added Redis, SQL Server, SARIF, and per-project configuration. v0.3 added `fix` and `update` -
shipped, which rounds out the `new`/`add`/`doctor`/`fix`/`update` set v1.0 is built on. Still ahead,
in rough order of what's next:
