...
public sealed class PostgresFeatureInstaller : IFeatureInstaller
{
    public FeatureDefinition Feature { get; } = new()
    {
        Id = "postgres",
        Name = "PostgreSQL",
        Description = "Adds Npgsql, an EF Core DbContext, connection configuration, and migrations support.",
    };
        ...
        FeatureContext context, CancellationToken cancellationToken = default)
    {
        ...
        DotnetCli.Run(["add", infrastructureCsproj, "package", "Npgsql.EntityFrameworkCore.PostgreSQL"], context.RootPath);
        steps.Add("Added Npgsql");
        ...
        // Npgsql and EFCore.Design can independently resolve to different Microsoft.EntityFrameworkCore
        // patch versions, which breaks the build once Api references Infrastructure. Pin both projects
        // to the same resolved version so the whole solution agrees on one EF Core.
    ...
    private static void WireConnectionConfiguration(FeatureContext context, string apiProjectPath)
    {
        var connectionString = $"Host=localhost;Database={context.ProjectName};Username=postgres;Password=postgres";
        AppSettingsEditor.AddConnectionString(
            Path.Combine(apiProjectPath, "appsettings.json"), "Postgres", connectionString);
