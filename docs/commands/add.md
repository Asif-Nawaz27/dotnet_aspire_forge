...
| `docker` | (Re)generates `Dockerfile`, `.dockerignore`, and `docker-compose.yml`, wired to whatever databases/caches are already configured. Safe to re-run after adding `postgres`/`redis` to refresh the compose file. |
